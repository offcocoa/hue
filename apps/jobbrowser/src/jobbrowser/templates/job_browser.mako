            ...
            <div class="content-panel-inner">
              ...
              <form class="form-inline">
                ...
                <!-- ko if: $root.interface() !== 'schedules' && $root.interface() !== 'bundles' -->
                  ...
                  <a class="btn" title="${ _('Create cluster') }" data-bind="visible: $root.compute() && $root.compute()['type'].indexOf('altus') >= 0, toggle: jobs.createClusterShow">
    ...
    var JobBrowserViewModel = function () {
      ...
      self.availableInterfaces = ko.pureComputed(function () {
        var jobsInterfaceCondition = function () {
          return self.appConfig() && self.appConfig()['browser'] && self.appConfig()['browser']['interpreter_names'].indexOf('yarn') != -1 && (!self.compute() || self.compute()['type'].indexOf('altus') == -1);
        };
        var dataEngInterfaceCondition = function () {
          return self.compute() && self.compute()['type'] == 'altus-de';
        };
        var dataWarehouseInterfaceCondition = function () {
          return self.compute() && self.compute()['type'] == 'altus-dw';
        };
        var dataWarehouse2InterfaceCondition = function () {
          return self.compute() && self.compute()['type'] == 'altus-dw2';
        };
        var schedulerInterfaceCondition = function () {
          return '${ user.has_hue_permission(action="access", app="oozie") }' == 'True' && (!self.compute() || self.compute()['type'].indexOf('altus') == -1);
        };
